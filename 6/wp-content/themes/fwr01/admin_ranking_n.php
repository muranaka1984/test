...
<img src="/analytics/img/admin_logo_n.png" width="100">
...
//  'order' => 'ASC',
//  'meta_key'=>'ランキング【ナイトラバー】',
//  'orderby'=>'meta_value',
); ?>
<?php query_posts( $args ); ?>

<form action="./admin.php?page=ranking_n" method="post">
...
<?php
  ...
  if(isset($_POST['update'])) {
    update_post_meta($cid, 'ランキング【ナイトラバー】', $input_data);
  }
  else if(isset($_POST['reset'])) {
    update_post_meta($cid, 'ランキング【ナイトラバー】', '');
  }
?>
<li class="ranking_list sort_<?php echo post_custom('ランキング【ナイトラバー】');?>" <?php if(post_custom('ランキング【ナイトラバー】')): ?>id="<?php echo post_custom('ランキング【ナイトラバー】');?>"<? else: ?>id="99"<? endif; ?>>
<p class="rank"><span class="number">
<?php if(post_custom('ランキング【ナイトラバー】')): ?>
  <?php echo post_custom('ランキング【ナイトラバー】');?> 
<? else: ?>
　
<? endif; ?>
</span>
...
<p class="input"><input type="text" name="g_rank<?php echo get_the_ID(); ?>" class="g_rank<?php echo get_the_ID(); ?>" size="2" style="ime-mode:disabled;" value="<?php echo post_custom('ランキング【ナイトラバー】'); ?>"><small>位</small></p>
</li>
